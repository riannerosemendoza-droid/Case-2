...
def compare_loop_and_numpy(values :np.ndarray) -> dict:
    ...
    loop_median = np.meadian(loop_times)
    ...
    return {
        ...
        "loop_meedian_time": loop_median,
        "numpy_median_time": numpy_median
    }

def run_numpy_analysis(df: pd.DataFrame) -> dict:
    """Run NumPy arrat, mask, vectorized calculation, and timing."""
    ...
    vectorized_total = np.sum(positive.values)

    results = compare_loop_and_numpy(values)
